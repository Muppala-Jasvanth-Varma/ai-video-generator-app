...
import { writeFile, rename, lstat as lstat2, mkdir, stat as stat2 } from "fs/promises";
...
async function parseSingleFile(path2, params) {
  ...
  const secondResp = await downloadFile({ ...params, path: path2, range: [8, 7 + Number(lengthOfHeader)] });
  if (!secondResp) {
    throw new SafetensorParseError(`Failed to parse file ${path2}: failed to fetch safetensors header.`);
  }
  try {
...
async function getAccessToken(repoId, initialAccessToken, customFetch, hubUrl) {
